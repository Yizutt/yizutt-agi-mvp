...
- 本地 Web 面板支持查看 Runtime 状态、提交任务并流式显示 trace、查看最近记忆和技能摘要，并支持多语言切换。
...
- `python/yizutt_agi/panel.py` 提供本地 Web 面板服务，把面板 API 代理到 Runtime CLI，并通过 SSE 桥接流式任务输出。
- `python/yizutt_agi/real_loop.py` 负责直接跑一次模型-记忆-技能闭环。
- `python/yizutt_agi/client.py` 是 Python 调 Rust Runtime CLI 的简单客户端。
- `web/panel/index.html` 是本地面板的浏览器页面，包含实时任务 trace 输出。
...
然后在浏览器打开 `http://127.0.0.1:50280`。面板支持编辑 Runtime 地址、查看 Worker 状态、提交任务、查看最近记忆和技能摘要。任务提交会通过 `/api/submit-stream` 把 `submit --stream` 桥接成浏览器 SSE 输出，因此工具调用、工具结果和最终 trace 会在 Worker 运行时实时显示。默认语言是中文-简体，可切换中文-繁体、英语、日语、韩语、阿拉伯语、俄语。模型 API key 只保留在服务端环境变量中，不会暴露给浏览器。
...
- 本地 Web 面板的状态、流式任务提交、记忆、技能 API 和多语言切换
...
- 在 Web 面板中增加持久任务历史和 replay。
- 增加更完整的工具取消机制和沙箱隔离。
- 用容器或 OS sandbox 加强 Worker 隔离。
- 增强图谱推理和技能排序。
- 扩展 CI，增加 Web 面板 smoke 检查。
