...
Nexus AGI MVP 是一个可运行的进化型终端 Agent 运行时原型。它用 Rust 实现本地 gRPC Runtime、Worker 进程管理和 WorkerPool 调度，用 Python sidecar 负责真实任务执行、模型调用、工作记忆和技能文件沉淀。
...
- `python -m nexus_agi.real_loop` 可以不启动 Rust Runtime，直接跑一次“任务-记录-存技能”闭环。

## 目录结构

- `proto/nexus.proto` 定义 `RuntimeService` 和 `WorkerService`。
- `crates/nexus-runtime` 是 Rust Runtime、Worker 进程、CLI 客户端和 WorkerPool。
- `python/nexus_agi/executor.py` 是 Rust Worker 启动的 Python sidecar。
- `python/nexus_agi/model_gateway.py` 是统一模型网关。
- `python/nexus_agi/memory.py` 是 SQLite FTS5 工作记忆。
- `python/nexus_agi/skills.py` 负责把技能保存为 `SKILL.md`。
- `python/nexus_agi/real_loop.py` 负责直接跑一次模型-记忆-技能闭环。
- `python/nexus_agi/client.py` 是 Python 调 Rust Runtime CLI 的简单客户端。
...
`target/debug/nexus-runtime submit --task "总结这个项目的架构"`
...
`python -m nexus_agi.real_loop --provider openai --task "用五点总结 Nexus AGI MVP 架构"`
...
`python -m nexus_agi.real_loop --provider anthropic --task "用五点总结 Nexus AGI MVP 架构"`
...
`python -m nexus_agi.real_loop --provider openai --task "完成一次 Nexus AGI 任务-记忆-技能闭环"`

当 `NEXUS_OPENAI_BASE_URL` 不是官方 OpenAI 地址时，Nexus 会自动走 chat completions。认证优先读取 `OPENAI_API_KEY`，如果没有则回退读取 `PROXY_API_KEY`。密钥只从环境变量读取，不会打印。
...
运行产生的数据库和技能输出位于 `.nexus/`，该目录已被 Git 忽略。
