...
产品方向已固定为三条主线：Codex 的功能和执行逻辑作为核心 Agent Runtime；OpenClaw 的 Web 与命令体验作为 operator 入口；Hermes 的记忆和学习能力作为长期成长层。

## 二、已完成的核心特性

- **项目更名**：对外项目名已从旧名称迁移为 Yizutt AGI。
- **产品方向三支柱**：README、中文说明、`yizutt --help` 和 `yizutt onboard` 已明确 Codex 风格功能/逻辑、OpenClaw 风格 Web/命令、Hermes 风格记忆/学习能力三条产品主线。
...
| 全局 CLI | `python/yizutt_agi/cli.py` | `yizutt` 入口，裸命令默认启动本地 Runtime/Web 工作台，并分发 `setup`、`onboard`、`gateway`、`skill`、`start` 等子命令；`onboard` 输出产品三支柱 |
| 能力与进化注册表 | `python/yizutt_agi/capabilities.py` | 统一记录 Codex/OpenClaw/Hermes/进化能力状态、验收标准和自进化任务生成逻辑 |
...
- **Codex 作为核心执行参照**：规划、工具循环、trace、sandbox 策略、任务 handoff 和 Worker 语义优先按严肃 coding agent Runtime 打磨。
...
- 从 `/tmp` 等仓库外路径执行 `yizutt start --dry-run` 能解析到仓库根目录。
...
### N3-0.4 已完成：交互式 `yizutt setup` 初始化配置

**目标**：仿 OpenClaw 的本地初始化体验，提供 `yizutt setup` 交互式配置入口，生成后续启动会实际读取的本地配置。

**完成情况**：`python/yizutt_agi/cli.py` 新增 `setup` 子命令。`yizutt setup` 会引导配置 Runtime host/port、Worker base port、mock 模型端口、Worker 数量、恢复策略、Workbench 端口、启动是否构建、模型 provider（local/openai/anthropic/openai-compatible）和可选高级路径；默认写入 `.yizutt/config.json`。支持 `--yes` 使用默认值、`--dry-run` 只打印不写入、`--json` 机器可读输出、`--config` 指定配置路径。裸 `yizutt`、`onboard` 和 `gateway` 已接入该配置，优先级为命令行参数 > 环境变量 > `.yizutt/config.json` > 内置默认值；密钥只通过环境变量读取，不写入配置。

**验收标准**：
- `yizutt setup --yes --dry-run` 可从仓库外路径输出完整配置。
- 指定 `--config <tmp>` 的 `yizutt setup --yes` 会写入 JSON，并被 `yizutt --dry-run --config <tmp>` 读取。
- `yizutt onboard --config <tmp> --json` 和 `yizutt gateway --config <tmp> --json` 可读取同一配置。
- Python 编译、Rust workspace 检查、CLI 子命令 smoke 通过。

### N3-0.5 已完成：Codex/OpenClaw/Hermes 产品方向固化

**目标**：把产品方向明确为 Codex 的功能和执行逻辑、OpenClaw 的 Web 与命令体验、Hermes 的记忆和学习能力，并让文档和 CLI 输出保持一致。

**完成情况**：README、中文说明和本上下文已增加三支柱产品方向。`python/yizutt_agi/cli.py` 新增 `PRODUCT_PILLARS`，`yizutt --help` 展示产品形态，`yizutt onboard` 和 `yizutt onboard --json` 输出 Codex/OpenClaw/Hermes 三条主线，便于后续产品化任务按这三类能力验收。

**验收标准**：
- README 和中文说明明确三条产品主线。
- `yizutt --help` 能看到 Codex/OpenClaw/Hermes 产品形态。
- `yizutt onboard` 和 `yizutt onboard --json` 能输出三支柱信息。
- Python 编译、Rust workspace 检查、CLI smoke 通过。

### N3-0.6 已完成：能力矩阵与自进化任务规划

**目标**：让 Yizutt 不只描述“拥有 Codex/OpenClaw/Hermes 能力”，而是能在 CLI 和 Web 中持续追踪能力状态，并把缺口转换为下一步自进化任务。

**完成情况**：新增 `python/yizutt_agi/capabilities.py`，统一维护 Codex、OpenClaw、Hermes 和 Evolution 四类能力、实现状态、验收标准、证据和下一步目标。`yizutt capabilities` 可输出能力矩阵，`yizutt evolve` 会按优先级从 partial/planned 能力生成自进化任务，可用 `--write` 写入 `.yizutt/evolution/next_plan.json`。Web 面板新增 `/api/capabilities` 和 `/api/evolution-plan`，右侧检查器新增“能力与进化”面板，展示当前能力进度和下一步任务。

**验收标准**：
- `yizutt capabilities` 和 `yizutt capabilities --json` 可查看四类能力矩阵。
- `yizutt evolve` 和 `yizutt evolve --json` 能输出下一步自进化任务。
- `yizutt evolve --write` 能写入 `.yizutt/evolution/next_plan.json`。
- Web 面板 API `/api/capabilities` 与 `/api/evolution-plan` 可返回同一能力数据。
